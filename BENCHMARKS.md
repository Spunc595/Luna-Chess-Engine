...
## Cycle close: v3.1.7 (2026-09-25)

The search-strength cycle that began at `v3.1.6` (`a907c7a`) ends here. One patch entered. Every SPRT below: bounds
[0, 10], alpha = beta = 0.05, 10+0.1, no book, concurrency 2, Block 0 adjudication, 0 games lost on time.

| block | change | base | outcome |
|---|---|---|---|
| G1 | TT move scored above every capture | `base` | rejected, 1,955 games, -3.4 Elo, LLR -2.95 |
| G2 | King attacker value 1000 in capture ordering | `base` | cap (12,000 games), +6.2 +/- 4.1, LOS 99.8%, suspended candidate |
| G3 | losing-capture history capped at 1000 | `base` | rejected, 966 games, -11.5 Elo (ran without the stop-rule: cabled into `sprt_match3.sh`) |
| **D3** | **akimbo's material scale after the network** | `base` | **accepted, 390 games, +44.8 Elo (stopping-biased, not publishable), LLR 2.95** |
| G5 | history tables persist across `go` | `base_d3` | rejected, 961 games, -10.8 Elo, LLR -3.00 |
| G4 | continuation history | -- | closed by Clause 3 after G5 (not re-measured, not queued) |
| D4 | the four cp margins / 0.840 | `base_d3` | stop-rule at 2,202 games, +0.3 +/- 9.6, LOS 52.6%: closed |
| G2b | G2 re-measured | `base_d3` | stop-rule at 2,405 games, +0.4 +/- 9.3, LOS 53.7%: closed |

**Final `head` = `base_d3`** (= `v3.1.7`). Not run: Block C (`c-improving-and-nmp-bonus`, written and tested, not
queued) and Block 6 (staged, not launched). No Elo figure for D3 is published: the +44.8 is the estimate of an SPRT
that stopped at 390 games, the worst case for upward bias; Block 6 will produce the fixed-length number.

**A lesson for anything that is measured on a moving base.** G2 measured +6.2 Elo with LOS 99.8% on `base` and +0.4
Elo on `base_d3`. Whatever the cause (the two changes act on the same phase of the game, or the first figure was a
chance excursion: the intervals do not separate the two readings), the same patch gave two different numbers on two
bases, and that is the practical reason the base moves with every accepted patch: a patch's figure belongs to the
base it was measured on.

### Release record

- Merge `89ca29b` (`d3-material-scale` into `main`, `--no-ff`), version bump `b93f9d8`, annotated tag `v3.1.7` on
  `b93f9d8`, pushed. The version string appears in exactly four places, all changed: `Cargo.toml`, `Cargo.lock`, the
  banner in `src/main.rs` (`Luna CE v3.1.7`) and the UCI `id name` line in `src/main.rs`; `printf 'uci\n' | luna` says
  `id name Luna CE v3.1.7`. 54 tests green (`cargo test --workspace --release`), 0 warnings.
- Android arm64, built from a clean `git archive` of the tag (NDK r27d, `aarch64-linux-android21`):
  `luna-v3.1.7-android-arm64`, sha256 `7ca8faf4c2f596a1b1192e1974d8b6f4c5cbdcc7c315cd0184ad09c637769015`
  (6,955,048 bytes; AArch64 ELF, interpreter `/system/bin/linker64`).
- Source archive built with `git -c core.autocrlf=false archive` (what GitHub generates from the stored blobs):
  `luna-v3.1.7-source.tar`, sha256 `df6ff9bcd4dc62bdc23f10f1c079758bc8a16c0e8cecaee55f1d1e876c46d636`, a reference
  copy; the archives GitHub attaches to the release are generated by GitHub and their bytes differ.
- **Line endings, checked and not as assumed.** `.gitattributes` only declares `*.sh` and `*.patch`. On this machine
  (`core.autocrlf=true`) a plain `git archive` converts to CRLF (41 text files), which is not what is stored. The
  stored blobs are LF except five source files that are **entirely CRLF, in `v3.1.6` too, unchanged by this release**:
  `src/evaluation.rs`, `src/lib.rs`, `src/main.rs`, `src/search.rs`, `src/zobrist.rs` (consistent CRLF, not mixed,
  cargo does not care). Normalising them would rewrite four large files for blame purposes; left alone, not part of
  this release.
- Bot (Oracle, `luna-bot.service`): binary built on Oracle from the tag sources, `f817914f552cfbe717793ec568e2f24ba7eddfa3190e6e74a3b8a84cc6d36d38`
  (`id name Luna CE v3.1.7`; `eval` on the start position 69 cp and on the suite pawn endgame 1168 cp, the same
  values as the patched PC build and akimbo's own binary). The previous binary (`v3.1.6`, sha256 `bec60041...eca06`) is kept as
  `engines/luna.backup-pre-v3.1.7`, next to the older backups. **The book was not touched.** `config.yml` has an
