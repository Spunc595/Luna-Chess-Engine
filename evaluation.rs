...
pub fn evaluate(board: &Scacchiera, params: &EvalParams) -> i32 {
    ...
    // Il mop-up (spingere il Re avversario verso l'angolo quando si è
    // ampiamente vinti) NON vive più qui: è stato generalizzato in
    // `search::apply_progress_adjustment`, che lo applica in `eval()`
    // indipendentemente dalla fonte della valutazione (questa PST classica
    // o la NNUE). Applicarlo anche qui lo conterebbe due volte quando la
    // rete non è caricata e si ricade su questa funzione.
