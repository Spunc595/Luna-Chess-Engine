...
positions = [("suite:middlegame(d18)", "position startpos moves " + moves, 18),
             ("suite:pawn endgame(d18)", "position fen 8/5pk1/6p1/8/1P6/P4PKP/8/8 w - - 0 40", 18)]
...
import os
def run(pos, depth, trace=None):
    env = dict(os.environ)
    if trace: env["LUNA_TRACE"] = trace
    p = subprocess.Popen([exe], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, encoding="utf-8", errors="replace", env=env)
    out, err = [], []
    ...
    d = {}
    for pre in ("PROF ", "PROFX"):
        line = [l for l in err if l.startswith(pre)][-1]
        d.update({k: float(v) for k, v in (kv.split("=") for kv in line.split()[1:])})
    return d

TRACE = sys.argv[3]
rows = []
for name, pos, d in positions:
    r = run(pos, d, TRACE if name.startswith("suite:middlegame") else None); r["name"] = name; rows.append(r)
