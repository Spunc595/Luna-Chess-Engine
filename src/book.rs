...
impl OpeningBook {
    ...
    pub fn load(path: &str) -> Option<Self> {
        ...
        for line in reader.lines() {
            if let Ok(l) = line {
                ...
                // A minimal FEN for the book has 4 parts + the move
                if parts.len() >= 2 {
                    let move_str = parts.last().unwrap().to_string();

                    // Take the FEN part (everything except the last word)
        ...
        println!("📚 Book: Loaded {} positions in memory.", count);
        Some(OpeningBook { entries })
    }

    /// Looks up a move for the current position
    pub fn get_move(&mut self, board: &mut Scacchiera) -> Option<Mossa> {
        ...
        // Strategy 1: exact key (including en passant target if present)
        let key_exact = parts[0..4].join(" ");

        // Strategy 2: "no en passant" key (force the "-" dash)
        // Many books (like our txt one) store "-" even when an EP target exists.
        let mut parts_no_ep = parts[0..4].to_vec();
        parts_no_ep[3] = "-";
        let key_no_ep = parts_no_ep.join(" ");

        // Try the exact key first, fall back to the no-EP one
        let candidate_moves = self.entries.get(&key_exact)
            .or_else(|| self.entries.get(&key_no_ep));

        if let Some(move_list) = candidate_moves {
            // Generate legal moves to validate against
            let legali = board.genera_mosse_legali(&ZobristKeys::default());

            // Filter: keep only book moves that are legal
